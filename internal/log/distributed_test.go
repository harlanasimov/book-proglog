...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		config.Raft.LeaderLeaseTimeout = 45 * time.Millisecond
	...
	got1, err := logs[1].Read(off)
	require.IsType(t, api.ErrOffsetOutOfRange{}, err)
	require.Nil(t, got1)

	got2, err := logs[2].Read(off)
	require.NoError(t, err)
	require.Equal(t, []byte("third"), got2.Value)
	require.Equal(t, got2.Offset, off)
}
