...
func (r *Resolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	//r.logger.Debug("Resolver.Build:", zap.Any("serviceConfig", r.serviceConfig))
...
func (r *Resolver) ResolveNow(resolver.ResolveNowOptions) {
	...
	for _, server := range res.Servers {
		//r.logger.Debug("Resolver.ResolveNow:", zap.String("RpcAddr", server.RpcAddr), zap.Bool("IsLeader", server.IsLeader))
		addrs = append(addrs, resolver.Address{
			Addr:       server.RpcAddr,
			Attributes: attributes.New("is_leader", server.IsLeader),
		})
	}
	//r.logger.Debug("Resolver.ResolveNow:", zap.Any("addrs", addrs))
